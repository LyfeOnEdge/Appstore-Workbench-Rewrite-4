...
class libget_handler(object):
    ...
    def install_package(self, package: dict, silent=False):
        if not self.check_path(): return warn_path_not_set()
        if not package: return print("No repo entry data passed to libget handler.\nNot continuing with install")
        try: package_name = package["name"]
        except: return print("Error - package name not found in repo data\nNot continuing with install")
        try: version = package["version"]
        except: return print("Error - package version not found in repo data\nNot continuing with install")
        title = f"Installing {package_name}"
        if version: title += f" - {version}"
        if not self.check_if_get_init(): return print("Get folder not initiated.\nNot continuing with install")
        if package_name in self.get_packages(silent=True): # Uninstall if already installed
            print("Package already installed, removing for upgrade")
            if not self.uninstall_package(package): return print("Uninstall failed.\nNot continuing with install")
        else: print("Package not previously installed, proceeding...")
        print(f"Beginning install for package {package_name}")
        # Append base directory to packages directory
        packagesdir = os.path.join(self.base_install_path, self.libget_dir)
        if not os.path.isdir(packagesdir): os.makedirs(packagesdir)
        # Append package folder to packages directory
        packagedir = os.path.join(packagesdir, package_name)
        if not os.path.isdir(packagedir): os.mkdir(packagedir)
        # Download the package from the switchbru site
        package_zip = self.webhandler.getPackage(package_name)
        if not package_zip: return print(f"Failed to download zip for package {package_name}")
        with ZipFile(package_zip) as zipObj:
            namelist = zipObj.namelist()
            if not PACKAGE_MANIFEST in namelist: # Easy check to see if info and manifest files are in the zip
                return print("Failed to find package manifest in zip... Stopping install, no files have been changed on the SD card")
            if not PACKAGE_INFO in namelist:
                return print("Failed to find package info in zip... Stopping install, no files have been changed on the SD card")
    ...
    def handler_install_package(self, package: dict, progress_function = None, reload_function = None, title_function = None, silent = False):
        ...
        if reload_function: reload_function()
    ...
    def edit_info(self, package_name: str, key: str, value):
        ...
        try:
            with open(pkg, encoding="utf-8") as infojson: info = json.load(infojson)
        except Exception as e: return print(f"Failed to open info data for {package_name} - {e}")
        info[key] = value
    ...
    def clean_version(self, ver, name):
        ...
        if name: ver = ver.replace(name.lower(), "")
