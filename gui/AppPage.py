...
from.NewDetailPage import DetailPage
...
class AppPage(ttk.Frame):
	def __init__(self, controller, repo, *args, **kwargs):
		...
		self.notfound_image = load_image_object_from_bytes_array(notfound_image)
	...
	def refresh(self, event = None):
		...
		try: filler_space = filler_space / (maxperrow + 1)
		...
		for t in self.tiles:
			...
			self.controller.threader.add(handle_tile, [t, place_x, place_y, this_ds])
			i += 1; _x += 1
			if _x == maxperrow:
				_x = 0
				_y += 1
	...
	def place_tile(self, tile):
		try:
			...
			title = tile.package["title"] if len(tile.package["title"]) < 33 else tile.package["title"][:20]+"..."
			author = tile.package["author"] if len(tile.package["author"]) < 26 else tile.package["author"][:23]+"..."
			version = tile.package["version"] if len(tile.package["version"]) < 25 else tile.package["version"][:17]+"..."
			tile.references.extend([
				self.canvas.create_rectangle(tile.x, tile.y, tile.x + tile.width, tile.y + tile.height, fill="#7F7F7F"),
				self.canvas.create_line(tile.x + 10, tile.y + 130, tile.x + tile.width - 10, tile.y + 130, width = 4),
				self.canvas.create_text(tile.x + 5, tile.y + 135, anchor = "nw", text = title),
				self.canvas.create_text(tile.x + 5, tile.y + 150, anchor = "nw", text = f"By - {author}"),
				self.canvas.create_text(tile.x + 5, tile.y + 165, anchor = "nw", text = f"Version - {version}"),
				self.canvas.create_text(tile.x + 5, tile.y + 180, anchor = "nw", text = f"Downloads - {downloads}"),	
				])
			if tile.active: self.activate_tile()

			if not tile.thumbnail:
				icon = self.repo.getPackageIcon(tile.package["name"])
				tile.thumbnail = Image.open(icon) if icon else self.notfound_image
	...
	def activate_tile(self, tile):
		tile.active_references.extend([
			self.canvas.create_rectangle(tile.x, tile.y, tile.x + tile.width, tile.y + tile.height, outline="#000000", width = 4),
			self.canvas.create_rectangle(tile.x + 4, tile.y + 4, tile.x + tile.width - 5, tile.y + tile.height - 5, fill="#424548"),
			self.canvas.create_text(tile.x + 8, tile.y + 8, text = tile.package["description"], anchor = "nw", width = tile.width - 16, fill = "#CCCCCC")
		])

	def deactivate_tile(self, tile):
		for r in tile.active_references: self.canvas.delete(r)
